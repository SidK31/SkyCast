...
function renderQuickCities() {
  let box = document.querySelector("#quickCities");
  if (!box) {
  ...
  if (!recentCities.length) {
    box.innerHTML = `<div class="section-heading"><h3>Quick access</h3><span>Your recent cities will appear here</span></div>`;
    return;
  }
  box.innerHTML = `<div class="section-heading"><h3>Quick access</h3><span>Recent searches</span></div><div class="saved-cities">${recentCities.map(city => `<button class="saved-city" data-city="${city.replace(/"/g, '&quot;')}">📍 ${city}</button>`).join("")}<button class="clear-cities" id="clearCities">Clear</button></div>`;
  ...
  result.innerHTML = `
    <section class="current-card glass">
      <div class="current-main">
        <div><p class="eyebrow">CURRENT WEATHER</p><h2>${place.name}${place.country_code ? `, ${place.country_code}` : ""}</h2><p class="condition">${info[0]}</p><div class="temperature">${temperature(current.temperature_2m)}<span>${isCelsius ? "C" : "F"}</span></div><p class="feels">Feels like ${temperature(current.apparent_temperature)}</p></div>
